...
namespace WickedSick
{
  ...
  void reflectionShaderCallback(Graphics* gSys, void* data, std::vector<ParamPasser>& params)
  {
    ...
    ReflectComponent* refComp = (ReflectComponent*) comp->GetSibling(CT_ReflectComponent);
    Shader* shader = callbackParams->shader;


    for(int i = 0; i < 6; ++i)
    {
      shader->AddTexture(gSys->GetRenderTarget(i)->GetTexture());
    }
    ...
    if(comp)
    {
      ...
      float refCoeff = refComp->GetReflectionCoeff();
      float refractCoeff = refComp->GetRefractionCoeff();
      params.push_back(std::move(ParamPasser("reflectionInput", "reflectionCoeff", &refCoeff, sizeof(refCoeff))));
      params.push_back(std::move(ParamPasser("reflectionInput", "refractionCoeff", &refractCoeff, sizeof(refractCoeff))));
  ...
  void skyboxShaderCallback(Graphics* gSys, void* data, std::vector<ParamPasser>& params)
  {
    ...
    shader->AddTexture(gSys->GetTexture("-x"));
    shader->AddTexture(gSys->GetTexture("+x"));
    shader->AddTexture(gSys->GetTexture("-y"));
    shader->AddTexture(gSys->GetTexture("+y"));
    shader->AddTexture(gSys->GetTexture("-z"));
    shader->AddTexture(gSys->GetTexture("+z"));
  ...
  void vertexShaderCallback(Graphics* gSys, void* data, std::vector<ParamPasser>& params)
  {
    ...
    ModelComponent* comp = callbackParams->modelComp;
    Shader* shader = callbackParams->shader;
    RenderTarget* target = gSys->GetRenderTarget(0);
    ...
    {
      shader->AddTexture(target->GetTexture());
      //once = true;
    }
  ...
  void ParticleShaderCallback(Graphics* gSys, void* data, std::vector<ParamPasser>& params)
  {
    ...
    EmitterDescription& emitterDesc = particleParam->emitter->GetDescription();
    ...
    if(particle)
    {
      ...
      particlePos += system->GetDescription().position;
      particlePos += tr->GetPosition();
      Matrix4 modelToWorld;
      modelToWorld.Identity();
      Camera* camera = gSys->GetCamera();


      Matrix4 scale = modelToWorld.GetScaled(particle->Scale);
      Matrix4 translate = modelToWorld.GetTranslated(particlePos);
      modelToWorld = translate * scale;
      ...
      params.push_back(std::move(ParamPasser(bufferName, paramName, &modelToWorld, sizeof(modelToWorld))));
      paramName = "worldToCamera";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &worldToCamera, sizeof(worldToCamera))));
      paramName = "cameraToClip";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &cameraToClip, sizeof(cameraToClip))));


      bufferName = "Particle";
      paramName = "Position";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Position, sizeof(particle->Position))));
      paramName = "Velocity";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Velocity, sizeof(particle->Velocity))));
      paramName = "Color";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Color, sizeof(particle->Color))));
      paramName = "Scale";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Scale, sizeof(particle->Scale))));
      paramName = "Rotation";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Rotation, sizeof(particle->Rotation))));
      paramName = "Lifetime";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->Lifetime, sizeof(particle->Lifetime))));
      paramName = "PrevPos";
      params.push_back(std::move(ParamPasser(bufferName, paramName, &particle->PrevPos, sizeof(particle->PrevPos))));
